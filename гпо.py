...
#X1 = np.pad(X, pad_width = (0, N))
nuliki = np.zeros ((j1, i2-1))
X1 = np.hstack((X, nuliki))
#X1 = [Х,np.zeros((j1,i2-1))]# Матрица Х дополненая нулями с правой стороны
k = j1
m = i1 + (i2 - 1)
y1 = np.zeros((k, m))
...
m = i1 + (i2 - 1)
y2 = np.zeros((k, m))
for k in range(0, j1-1):  # выбор строки обработки
    for m in range(0, (i1 + (i2 - 1)-1)):  # выбор стлобца обработки
...
X5=[X5,np.zeros((j1,i2-1))] #Матрица Х0 дополненая нулями с правой стороны
k = j1
m = i1 + (i2 - 1)
y5 = np.zeros((k, m))
...
X6=[y5,np.zeros((j1,i2-1))] #Матрица Х0 дополненая нулями с правой стороны
k = j1
m = i1 + (i2 - 1)
y6 = np.zeros((k, m))
for k in range(1, j1):  # выбор строки обработки
...
R = A;
u = 1;
i1 = y8.shape[1]  # кол-во столбцов в у8
j1 = y8.shape[0]  # кол-во строк в у8
number1 = int(q1 + j1 - 1)
number2 = int(q1 + i1 - 1)
Z1 = np.zeros((number1, number2))  # нулевая матрица размером внешней
Z1 = y8  # итоговая матрица
# -----------------------ПОДЪЕМ ЦЕНТРА---------------------
Y9 = np.ones((1, p))  # вид СР
...
X10 = np.zeros((j1 + (j2 - 1)-1), (i1-1))  # нулевая матрица
X10[1:j1,:] = y9
k = j1 + (j2 - 1)
m = i1
y10 = np.zeros((k, m))
